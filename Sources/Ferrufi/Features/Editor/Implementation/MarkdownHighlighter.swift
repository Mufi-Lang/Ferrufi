...
public final class MarkdownHighlighter {
    ...
    public init(theme: IronTheme, baseFontName: String?, baseSize: Double) {
        ...
        public func highlight(in storage: NSTextStorage, range: NSRange? = nil) {
            ...
            apply(pattern: #"^#{1,6}\s+.+$"#, type: .header, in: storage, text: text, range: highlightRange, options: .anchorsMatchLines)
    
            
    
            // Bold
    
            apply(pattern: #"\*\*.+?\*\*"#, type: .bold, in: storage, text: text, range: highlightRange)
    
            apply(pattern: #"__.+?__"#, type: .bold, in: storage, text: text, range: highlightRange)
    
            
    
            // Italic
    
            apply(pattern: #"\*.+?\*"#, type: .italic, in: storage, text: text, range: highlightRange)
    
            apply(pattern: #"_.+?_"#, type: .italic, in: storage, text: text, range: highlightRange)
    
            
    
            // Lists
    
            apply(pattern: #"^\s*[\*\-\+]\s+"#, type: .list, in: storage, text: text, range: highlightRange, options: .anchorsMatchLines)
    
            apply(pattern: #"^\s*\d+\.\s+"#, type: .list, in: storage, text: text, range: highlightRange, options: .anchorsMatchLines)
    
            
    
            // Code blocks & inline code
    
            apply(pattern: #"`{3}.*?\n[\s\S]*?\n`{3}"#, type: .code, in: storage, text: text, range: highlightRange)
    
            apply(pattern: #"`[^`]+`"#, type: .code, in: storage, text: text, range: highlightRange)
    
            
    
            // Links
    
            apply(pattern: #"\[.+?\]\(.+?\)"# , type: .link, in: storage, text: text, range: highlightRange)
    
            
    
            // Wiki Links
    
            apply(pattern: #"\[\[.+?\]\]"#, type: .wikiLink, in: storage, text: text, range: highlightRange)
