...
struct FolderCreationView: View {
    ...
    private var availableParentFolders: [Folder] {
        var folders = [ferrufiApp.folderManager.rootFolder]
        folders.append(
            contentsOf: ferrufiApp.folderManager.folders.values.sorted { $0.name < $1.name })
        return folders
    }
    ...
    private var previewPath: String {
        ...
        if let parent = selectedParentFolder {
            return "\(ferrufiApp.folderManager.fullPath(for: parent.id))/\(cleanName)"
        } else {
            return cleanName
        }
    }
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    navigationModel.selectFolder(newFolder, ferrufiApp: ferrufiApp)
