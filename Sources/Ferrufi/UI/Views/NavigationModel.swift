...
//  Ferrufi
//
//  Navigation state management for the Ferrufi knowledge management application
//
...
public class NavigationModel: ObservableObject {
    ...
    /// Reference to the main FerrufiApp instance
    public weak var ferrufiApp: FerrufiApp?
    ...
    public func selectNote(_ note: Note, ferrufiApp: FerrufiApp) {
        selectedNote = note
        selectedFolder = ferrufiApp.folderManager.folder(for: note)
    ...
    /// Selects a folder
    public func selectFolder(_ folder: Folder, ferrufiApp: FerrufiApp? = nil) {
        ...
        if let ferrufiApp = ferrufiApp {
            ferrufiApp.folderManager.selectFolder(folder)
        }
    }
    ...
    public func search(_ query: String) {
        ...
        searchDebouncer = Timer.scheduledTimer(withTimeInterval: 0.3, repeats: false) {
            [weak self] _ in
            Task { @MainActor in
                guard let self = self, let ferrufiApp = self.ferrufiApp else { return }
                await self.performSearch(with: ferrufiApp)
            }
        }
    }
    ...
    /// Creates a new note
    public func createNote(title: String, content: String = "", ferrufiApp: FerrufiApp) {
        showingNoteCreation = false

        Task {
            do {
                let newNote = try await ferrufiApp.createNote(title: title, content: content)
                await MainActor.run {
                    self.selectNote(newNote, ferrufiApp: ferrufiApp)
                }
            } catch {
                await MainActor.run {
    ...
    public func createFolder(name: String, parent: Folder? = nil, ferrufiApp: FerrufiApp) {
        ...
        Task {
            do {
                ...
                if let parent = parent {
                    ...
                    parentPath = ferrufiApp.folderManager.rootFolder.path
                }
                ...
                await MainActor.run {
                    ...
                    let newFolder = ferrufiApp.folderManager.createFolder(
                        name: name,
                        path: folderPath,
                        parentId: parent?.id
                    )
                    ...
                    self.selectFolder(newFolder, ferrufiApp: ferrufiApp)
                }
            } catch {
                await MainActor.run {
                    self.showError(error)
    ...
    private func performSearchInternal() async {
        ...
        guard let ferrufiApp = ferrufiApp else {
            searchResults = []
            return
        }

        await performSearch(with: ferrufiApp)
    }

    /// Performs search using FerrufiApp
    public func performSearch(with ferrufiApp: FerrufiApp) async {
        ...
        let results = await ferrufiApp.search(searchText)
        await MainActor.run {
