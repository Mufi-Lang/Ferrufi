...
final class LineNumberRulerView: NSRulerView {
    ...
    override func drawHashMarksAndLabels(in rect: NSRect) {
        ...
        while glyphIndex < endGlyph {
            ...
            let lineRect = layoutManager.lineFragmentUsedRect(forGlyphAt: glyphIndex, effectiveRange: &lineGlyphRange, withoutAdditionalLayout: true)
            ...
            let prefix = (textView.string as NSString).substring(with: NSRange(location: 0, length: charIndex))
...
struct MarkdownEditor: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        if let app = IronApp.shared, app.configuration.editor.showLineNumbers {
    ...
    func updateNSView(_ nsView: NSScrollView, context: Context) {
        ...
        // This code runs during view updates, so it will respond to configuration changes.
