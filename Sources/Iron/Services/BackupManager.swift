/*
 ...
 Notes:
 - DB file: ~/.iron/iron.db
 - Backups storage: ~/.iron/backups/<uuid>-<timestamp>.bak
...
public final class BackupManager {
    ...
    private init() {
        ...
        let ironDir = home.appendingPathComponent(".iron", isDirectory: true)
        try? FileManager.default.createDirectory(at: ironDir, withIntermediateDirectories: true)

        self.dbURL = ironDir.appendingPathComponent("iron.db")
    ...
    public func removeBackup(id: UUID) throws {
        ...
        do {
            ...
            throw BackupError.fileOperationError("Failed to remove backup file: \(error.localizedDescription)")
        }
    ...
    public func pruneBackups(forNoteId noteId: UUID, keep: Int) throws {
        ...
        let sql = "SELECT id FROM backups WHERE note_id = ? ORDER BY created_at DESC LIMIT -1 OFFSET ?;"
        var stmt: OpaquePointer?
        let rc = sqlite3_prepare_v2(db, sql, -1, &stmt, nil)
        guard rc == SQLITE_OK, let statement = stmt else {
            throw BackupError.sqliteError("prepare failed: \(sqliteErrorMessage())")
    ...
    /// Reschedule using the current IronApp configuration (if available)
    ...
    private func insertBackupRecord(
        ...
        let sql = "INSERT INTO backups (id, note_id, original_path, backup_path, created_at, size) VALUES (?, ?, ?, ?, ?, ?);"
        var stmt: OpaquePointer?
    ...
    private func lastBackupDate(forNoteId noteId: UUID) throws -> Date? {
        let sql = "SELECT created_at FROM backups WHERE note_id = ? ORDER BY created_at DESC LIMIT 1;"
        var stmt: OpaquePointer?
        guard sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK, let statement = stmt else {
            throw BackupError.sqliteError("prepare failed: \(sqliteErrorMessage())")
        }
